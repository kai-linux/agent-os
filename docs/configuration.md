...
3. **`backlog_groomer.py`** scans every repo for stale issues (>30 days), Known Issues in `CODEBASE.md` that don't have linked tickets, and risk flags from recent agent results. It refills the backlog toward a target depth (`plan_size * backlog_depth_multiplier`, default 2x) so the planner always has roughly twice the next sprint's worth of candidates to choose from. Each run is capped at 5 new issues; if the backlog is already at target the groomer skips generation. At least 40% of generated issues must be adoption-facing (demo, README, quickstart, public proof) — not internal infrastructure. Semantic deduplication (0.75 similarity threshold) prevents duplicate issues from piling up.

The planner and groomer are safe to invoke frequently from cron. Each repo has its own cadence in config (`sprint_cadence_days`, `groomer_cadence_days`), fractional days are supported, and `0` means dormant. The configured cadence is a minimum interval, not a scheduler by itself: the job only runs when cron invokes it, so cron must run at least as often as your shortest desired cadence. **`groomer_cadence_days` defaults to half the planner cadence** so the groomer runs ahead of the planner and can refill the backlog before the next sprint plan; set it explicitly to override. Backlog depth is configurable via `backlog_depth_multiplier` (relative to `plan_size`) or `target_backlog_depth` (absolute), at top-level or per-repo.
