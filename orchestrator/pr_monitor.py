...
def _send_risk_telegram(cfg: dict, repo: str, pr_number: int, risk: RiskAssessment):
    ...
    event = {
        ...
        "summary": f"High-risk PR #{pr_number}: {risk.short_summary}",
...
def _send_quality_harness_telegram(cfg: dict, repo: str, pr_number: int, failing_fixtures: list[str], score: float, threshold: float):
    event = {
        ...
        "summary": (
            f"Quality harness blocked PR #{pr_number}: score {score:.2f} below {threshold:.2f}; "
            f"fixtures: {', '.join(failing_fixtures[:5]) or 'unknown failure'}"
        ),
        "dedup_key": f"quality-harness:{repo}:{pr_number}",
    }
    severity = classify_severity(cfg, "pr_monitor", event)
    route_incident(severity, event, cfg=cfg)
